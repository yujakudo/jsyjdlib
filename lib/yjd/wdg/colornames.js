...
yjd.wdg.ColorNames = function(){
	this.colorList = [];
	for(var prop in this.ColorInfo) {
		this.colorList.push(prop);
	}
	this.listSort();
};

yjd.wdg.ColorNames.prototype.str2rgba = function(str) {
...
yjd.wdg.ColorNames.prototype.rgba2str = function(rgba) {
	var str;
	if(rgba[3]==1) {
		str = '#';
		...
		str = 'rgba(%1,%2,%3,%4)'.fill(rgba[0],rgba[1],rgba[2],rgba[3]);
	}
	return str;
}

yjd.wdg.ColorNames.prototype.getColAlph = function(col){
	if(typeof col==='string') col = this.str2rgba(col);
	var rgbstr = col.map(function(val){
		return ('0' + Math.round(val).toString(16)).substr(-2);
	});
	rgbstr.pop();
	return [ '#'+rgbstr.join(''), col[3] ];
};

yjd.wdg.ColorNames.prototype.distance = function(col1, col2) {
	var dist = 0;
	for(var i=0; i<3; i++) {
		dist += Math.pow(col1[i]-col2[i], 2);
	}
	return Math.sqrt(dist);
}
yjd.wdg.ColorNames.prototype.listSort = function(col){
	if(typeof col==='string') col = this.str2rgba(col);
	if(!col) col = [255,255,255,1];
	var info = this.ColorInfo;
	var distance = this.distance;
	this.colorList.sort(function(s1, s2){
		var d1 = distance(col, info[s1].rgb);
		var d2 = distance(col, info[s2].rgb);
		return (d1==d2)? 0: (d1<d2)? -1: 1;
	});
}
yjd.wdg.ColorNames.prototype.sortSelect = function(atm, col) {
	this.listSort(col);
	for(var i=0; i<this.colorList.length; i++) {
		var opt = yjd.atm('option[value="'+this.colorList[i]+'"]', atm);
		atm.append(opt);
	}
};

yjd.wdg.ColorNames.prototype.getOptions = function(col, ptn) {
	var rgba;
	if(col) {
		rgba = col;
		if(typeof rgba==='string') col = this.str2rgba(rgba);
		this.listSort(rgba);
	}
	if(ptn===undefined) ptn = 2;
	var template = this.getOptions.templates[ptn];
	var vals = {
		value:'transparent', name:'透過', textcolor:'black', selected: ''
	};
	if(col==='transparent') vals.selected = 'selected';
	var str = template.fill(vals);
	for(var i=0; i<this.colorList.length; i++) {
		var info = this.colorList[i];
		vals.value = info.name[0];
		vals.name = info.name[1];
		vals.selected = (col===info.name[0] ||
			(rgba && info.rgb[0]==rgba[0] && info.rgb[1]==rgba[1] && info.rgb[2]==rgba[2])
			)? 'selected': '';
	}
};
yjd.wdg.ColorNames.prototype.getOptions.templates = [
	'<options value="%value%" title="%name%" %selected%>%value%</options>',
	'<options value="%value%" title="%value%" %selected%>%name%</options>',
	'<options value="%value%" %selected%>%value%: %name%</options>',
];

yjd.wdg.ColorNames.prototype.hsl2rgb = function(hsl) {
...
yjd.wdg.ColorNames.prototype.Regex = {
	color24: {
		regx: /^\s*(#[\da-fA-F]{6})/,
	...
	color_hsl: {
		...
		func: function(matched) {
			...
			var rgba = yjd.wdg.ColorNames.prototype.hsl2rgb(hsl);
			rgba[3] = 1;
			return rgba;
		}
	},
...
yjd.wdg.ColorNames.prototype.ColorInfo = {
	white: {
		rgb: [255, 255, 255],
		name: ['white', '白']
	},
