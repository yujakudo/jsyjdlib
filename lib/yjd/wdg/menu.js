...
yjd.wdg.Menu.prototype.bind = function(options) {
	...
	function mouseover(event, atm) {
		...
		var onScrRect = new yjd.atm.rect(atmSub, scrRect);
		var itemRect = new yjd.atm.rect(data.atm);
		atmSub.style({ top:0, left:0});
		var subRect = new yjd.atm.rect(atmSub, itemRect);
		...
		for(var i=0; i<2; i++) {
			if(dir & 0x01) {	//	right
				if(data.childOfBar) subRect.right(0); else subRect.right(itemRect.w-borderWidth);
			} else {	//	left
				if(data.childOfBar) subRect.left(0); else subRect.left(itemRect.w-borderWidth);
			}
		...
		if(i===2) {
			b_narrow = true;
			subRect.x = itemRect.x;
			if(scrRect.x+scrRect.w<subRect.x+subRect.w) {
				onScrRect.right(0);
				if(onScrRect.x<0) onScrRect.left(0);
				subRect.x = onScrRect.x;
			}
		}
		for(i=0; i<2; i++) {
			if(dir & 0x10) {	//	up
				if(data.childOfBar || b_narrow) subRect.bottom(itemRect.h); else subRect.bottom(-borderWidth);
			} else {	//	down
				if(data.childOfBar || b_narrow) subRect.top(itemRect.h); else subRect.top(-borderWidth);
			}
		...
		if(i===2) {
			...
			subRect.y = onScrRect.y;
		}
		atmSub.style('top', subRect.top()+'px');
		atmSub.style('left', subRect.left()+'px');
		data.dir = dir;
	}
