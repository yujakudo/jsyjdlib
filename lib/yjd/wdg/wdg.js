...
/**
 * Attach to last of specified element and bind.
 * @param {yjd.atm|string|object|see} q yjd.atm object or parameter to create. @see yjd.atm
 * @param {@param {string|object|number} [context=document]} content @see yjd.atm
 */
yjd.wdg.prototype.attach = function(q, context) {
	if(!(q instanceof yjd.atm)) q = yjd.atm(q, context);
	q.append(this.atm);
	this.bind();
};
...
/**
 * Get selector string from objects.
 * @param {string|object|number} selector Parameter to create yjd.atm.
 * @ruturn {string} Selector string by id or class.
 */
yjd.wdg.getSelectorStr = function(selector, prefix) {
	if(typeof selector !== 'object') return selector;
	selector = yjd.atm(selector);
	if(!selector || !selector.elm) {
		return null;
	} else if(selector.elm.id) {
		selector = '#'+selector.elm.id;
	} else {
		var classes = selector.attr('class').split(' ');
		selector = null;
		selector = '.'+classes[0];
		for(var i=1; i<classes.length; i++) {
			if(!(prefix && classes[i].substr(0, prefix)!==prefix)
				&& !(classes[i].substr(0, 13)!=='yjd-editable-') ) {
				selector = '.'+classes[i];
				break;
			}
		}
 ...
 * @param {Object} [_this] Value of this to call callback.
 * 	if ommited it takes from options.this, or the function.
 */
yjd.wdg.prototype.callback = function(func, args, _this) {
	if(!func) return false;
	if(!_this && this.options.hasOwnProperty('this')) _this = this.options.this;
	if(!_this) _this = func;
