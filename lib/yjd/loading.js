 ...
 * depend on yjd.atm
 * @since  2018.02.04  initial coding.
 */
 ...
 */
 yjd.Loading = function(exp, options) {
	...
	if(!yjd.Loading.b_style) {
		yjd.atm.addStyle('@keyframes yjd-loading-frames',
		['{',
	...
	//	make the element
	this.atm = yjd.atm('<div class="yjd-wdg-loading"></div>');
	yjd.atm.check(exp)[this.options.function](this.atm);
	this.render();
...
yjd.Loading.prototype.show = function(s_mark) {
	...
	for(prop in this.options.marks) {
		...
		yjd.atm('.yjd-loading-'+prop, this.atm).style('visibility', 'hidden');
	}
	if(!s_mark) return;
	if(!(s_mark instanceof Array)) s_mark = [s_mark];
	for(var i=0; i<s_mark.length; i++) {
		yjd.atm('.yjd-loading-'+s_mark[i], this.atm).style('visibility', 'visible');
	}
};
...
yjd.Loading.prototype.render = function() {
	//	make canvas
	var canvas = yjd.atm('<canvas></canvas>');
	canvas.style({width:'100px', height:'2px'});
	this.ctx = canvas.elm.getContext('2d');
	//	container style
	...
	yjd.extend(containerStyle, this.options.style);
	this.atm.style(containerStyle);
	//	rener marks
...
yjd.Loading.prototype.renderMark = function(name, opt) {
	...
	if(opt.hasOwnProperty('animation')) {
		var optAnim = yjd.extend({}, this.options.marks.default.animation, opt.animation);
		yjd.extend(markStyle, {
	...
	var atmMark = yjd.atm('<div class="yjd-loading-'+name+'"></div>');
	this.atm.append(atmMark);
	//	text
	if(opt.hasOwnProperty('text')) {
		atmMark.html(opt.text);
		var fontSize = opt.hasOwnProperty('fontSize')?
				opt.fontSize: this.options.marks.default.fontSize;
		fontSize = this.percent2num(fontSize) * this.atm.elm.clientHeight;
	...
	//	set styles
	if(opt.hasOwnProperty('style')) yjd.extend(markStyle, opt.style);
	atmMark.style(markStyle);
	//	return if dots does not exist
	if(!opt.hasOwnProperty('dot')) return;
	// dots options
	var dotOpt = {};
	yjd.extend(dotOpt, this.options.marks.default.dot);
	yjd.extend(dotOpt, opt.dot);
	//	drow gradations.
	...
	var s_radius = ''+(rx)+'% / '+(ry)+'%'
	//	make dots
	var contWidth = ''+atmMark.elm.clientWidth+'px';
	for(var i=0; i<dotNum; i++) {
		var dot = yjd.atm('<div class="yjd-wdg-loading-dot"></div>');
		atmMark.append(dot);
		var ratio = i/dotNum;
		...
		var s_sift = ''+(atmMark.elm.clientWidth/2)+'px - '+align+'%';
		var s_isift = ' - '+(atmMark.elm.clientWidth/2)+'px + '+align+'%';
		var styles = {
		...
		dot.style(styles).style(dotOpt.style);
	}
	//
