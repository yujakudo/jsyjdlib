 ...
 * @license    MIT License
 * @fileoverview Load contents and replace directive with content.
 * depend on yjd.atm
 * @since  2017.04.07  initial coding.
 */

/**
 * Class to loard contents.
 * @constructor
 * @param {string} baseUrl Base URL to resolve relative path.
 */
yjd.Loader = function(baseUrl) {
	...
	this.errors = [];
	/** Base URL to resolve relative path	@type {string} */
...
yjd.Loader.prototype.doFilter = function(data, type) {
	type = yjd.typeInfo(type).synonym;
	if(type==='html') {
		this.parseHtml(data);
	}
};
 ...
 */
yjd.Loader.prototype.enque = function(item, type, atm, user) {
...
yjd.Loader.prototype.loadItem = function(idx) {
	...
	var url = yjd.getAbsoluteUrl(item.url, this.baseUrl);
	yjd.ajax(url, this)
