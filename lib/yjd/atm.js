...
yjd.atm.prototype.style = function(name, value) {
	...
	if(typeof name=='object') {
		for(var prop in name) {
			var stylename = yjd.atm.hyphen2camel(name);
 ...
 * @param {yjd.atm|string|HTMLElement} [context=window] Node to be base of axis.
 ...
 * @return {yjd.atm} yjd.atm object of a Child node. 
 */
yjd.atm.prototype.child = function(q) {
	...
	var atms = yjd.atms.children(q);
	return yjd.atms.item(0);
};
...
yjd.atm.getNodes = function (nodelist, selector) {
	...
	for(var i=0; i<nodelist.length; i++) {
		var test = this.nodelist[i];
		if(selector===undefined || nodelist[i].matches(selector)) elms.push(nodelist[i]);
	}
	return elms;
};
...
yjd.atm.prototype.removeChild = function(selector) {
	var matches = [];
	var elms = yjd.atm.getNodes(this.children, selector);
	for(i in elms) this.elm.removeChild(elms[i]);
	return this;
};
 ...
 * @return {yjd.atm} specified indexed element.
 */
 ...
 * </ul>
 * @constructor
 * @param {number|yjd.atm} x X. Or, element to get area.
 * @param {number|yjd.atm} [y] Y when x specified by number. 
 * 	Or, element to be context area.
...
yjd.atm.rect = function(x,y,w,h) {
	...
	if(typeof x==='object') {
		...
		this.h = rect.height;
		if(typeof y==='object') {
			this.context = new yjd.atm.rect(y);
		}
	} else {
		this.x = x;
		this.y = y;
 ...
 * @param {number} [value] Value to set this.y.
...
yjd.atm.rect.prototype.top = function(value) {
	...
	value = this.y;
	if(this.context) value -= this.context.top();
	return value;
};
 ...
 * If the instance has context, it returns value in context area.
 * @param {number} [value] Bottom value to set calcurated value to this.y.
 * @return {number} Value of bottom.
 */
yjd.atm.rect.prototype.bottom = function(value) {
	if(typeof value==='number') {
		this.y = value-this.h+1;
	}
	value = this.y+this.h-1;
	if(this.context) value = this.context.bottom() - value;
	return value;
};
 ...
 * If the instance has context, it returns value in context area.
 * @param {number} [value] Value to set this.x.
...
yjd.atm.rect.prototype.left = function(value) {
	...
	value = this.x;
	if(this.context) value -= this.context.left();
	return value;
};
 ...
 * If the instance has context, it returns value in context area.
 * @param {number} [value] Right value to set calcurated value to this.x.
 * @return {number} Value of right.
 */
yjd.atm.rect.prototype.right = function(value) {
	if(typeof value==='number') {
		this.x = value+1-this.w;
	}
	value =  this.x+this.w-1;
	if(this.context) value = this.context.right() - value;
	return value;
};
