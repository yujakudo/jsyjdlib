...
yjd.atm.prototype.val = function(value) {
	if(value===undefined) {
		if(this.elm.nodeName==='SELECT') {
			if(this.elm.selectedOptions.length==0) return null;
			value = [];
			for(var i=0; i<this.elm.selectedOptions.length; i++) {
				var opt = this.elm.selectedOptions[i];
				value.push(opt.value);				
			}
			if(this.elm.hasAttribute('multiple')) return value;
			return value[0];
		} else if(this.elm.getAttribute('type')==='checkbox') {
	...
	if(this.elm.nodeName==='SELECT') {
		...
		for(var i=0; i<this.elm.length; i++) {
			if(value.indexOf(this.elm[i].value)>=0) {
				this.elm[i].selected = true;
			} else {
				this.elm[i].selected = false;
			}
		}
	} else if(this.elm.getAttribute('type')==='checkbox') {
...
/**
 * Check has focus.
 * @return {boolean} True if it has focus, otherwise false.
 */
yjd.atm.prototype.hasFocus = function() {
	return this.elm === this.elm.ownerDocument.activeElement;	
}
 ...
 * @param {string} s_event Event type
 * 	@see https://developer.mozilla.org/en-US/docs/Web/Events
 * @param {boolean} [b_bubbling=true] Whetheer bubbling up event chain. 
 * @param {boolean} [b_cancelable=true] Whether be cancelable
 */
yjd.atm.prototype.trigger = function(s_event, b_bubbling, b_cancelable) {
	if(b_bubbling===undefined) b_bubbling = true;
	if(b_cancelable===undefined) b_cancelable = true;
	var event = document.createEvent("HTMLEvents");
	event.initEvent(s_event, b_bubbling, b_cancelable );
...
yjd.atms.prototype.each = function(o_this, selector, func) {
	...
	var imp_o_this = o_this;
	var imp_selector = selector;
	var imp_func = func;
	if(arguments.length===1) {
		imp_o_this = this;
		imp_selector = undefined;
		imp_func = arguments[0];
	} else if(arguments.length===2) {
		if(typeof arguments[0]==='object') {
			imp_o_this = arguments[0];
			imp_selector = undefined;
		} else {
			imp_o_this = this;
			imp_selector = arguments[0];
		}
		imp_func = arguments[1];
	}
	for(var i=0; i<this.elms.length; i++) {
		if(imp_selector===undefined || this.elms[i].matches(imp_selector)) {
			var atm = yjd.atm(this.elms[i]);
			if(false===imp_func.call(imp_o_this,atm)) break;
		}
	}
	return this;
};
...
/**
 * Set ot get value of input radio.
 * @param {string} [value] Value to set. omit to get value. 
 * @return {yjd.atms|string|null} The instance itself. Or, value of element if getting.
 */
yjd.atms.prototype.val = function(value) {
	if(value===undefined) {
		for(var i=0; i<this.elms.length; i++) {
			if(this.elms[i].checked) return this.elms[i].value;
		}
		return null;
	}
	for(var i=0; i<this.elms.length; i++) {
		if(this.elms[i].value!==value) continue;
		this.elms[i].checked = true;
	}
	return this;
};
 ...
 * Get style sheet.
 */
yjd.atm.styleSheet = function() {
	if(yjd.atm.styleSheet.object) return yjd.atm.styleSheet.object;
	yjd.atm('head').append('<style></style>');
	yjd.atm.styleSheet.object = document.styleSheets.item(document.styleSheets.length-1);
	return yjd.atm.styleSheet.object;
};
yjd.atm.styleSheet.object = null;

/**
 * Add rule to the style sheet.
 * @param {string} s_selector Selector or whole rule.
 * @param {sting|string[]|object} block style rule block.
 */
yjd.atm.addStyle = function(s_selector, block) {
	if(block instanceof Array) block = block.join(' ');
	if(typeof block==='object') block = yjd.atm.getStyleText(block);
	var sheet = yjd.atm.styleSheet();
	var idx = sheet.cssRules.length;
	sheet.insertRule(s_selector+block, idx);
};
...
/**
 * Get maximun number of last of id.
 * @param {string} selector Selector string
 * @param {string|number|object} context context
 * @param {string} delimiter Delimiter before number. if omitted, '-' takes.
 * @return {number} maximun value. if not found, return -1.
 */
yjd.atm.getMaxnumOfId = function(selector, context, delimiter) {
	if(delimiter===undefined) delimiter = '-';
	var atms = yjd.atms(selector, context);
	var max = -1;
	atms.each(function(atm){
		var id = atm.elm.id;
		if(id===undefined) return;
		if(id.lastIndexOf(delimiter)<0) return;
		var part = id.substr(id.lastIndexOf(delimiter)+1);
		var num = Number(part);
		if(max < num) max = num;
	});
	return max;
